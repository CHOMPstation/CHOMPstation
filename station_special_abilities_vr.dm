...
/mob/living/carbon/human/proc/face_sit()
	...
	*/
/*
/mob/living/carbon/human/proc/soul_vore()
	set name = "Soul Vore"
	set desc = "Invade the Body of the Prey and eat his soul"
	set category = "Abilities"
	var/souldrained=0
	if(last_special > world.time)
		return
	...
	if(!istype(G))
		to_chat(C, "<span class='warning'>You must be grabbing a creature in your active hand to foundle with their soul.</span>")
		return

	var/mob/living/carbon/human/T = G.affecting

	if(!istype(T) || T.isSynthetic())
		to_chat(src, "<span class='warning'>\The [T]'s soul is not able to be toyed with.</span>")
		return

	if(G.state != GRAB_NECK)
		to_chat(C, "<span class='warning'>You must have a tighter grip to get to the soul of this creature.</span>")
		return

	var/choice = input(src, "What do you wish to do to the soul?") as null|anything in list("Drain", "Steal")
	...
	if(do_after(src, 300, T)) //Thrirty seconds.
		if(choice == "Drain")
			src.show_message("<span class='warning'>You sink your fangs into [T]'s soul and start to drain it!</span>")
			src.visible_message("<font color='red'>[src] sinks their fangs into [T]!</font>")
			src.nutrition += 400
			T.brainloss() +=25
			src.show_message("<span class='warning'>You notice [T]'s soul is getting weaker, they dont look like it will survive another attack!</span>")
			souldrained++
			if(souldrained==2)
				T.stat=DEAD
				souldrained=0
			...
			src.show_message("<span class='warning'>You sink your fangs into [T]'s soul and pull out the Soul of [T] starting to devour it!</span>")
			src.visible_message("<font color='red'>[src] sinks their fangs into [T], pulling out a ghostly figure!</font>")
