...
/datum/admins/proc/unjobbanpanel()
	var/count = 0
	var/dat
	//var/dat = "<HR><B>Unban Player:</B> <font color='#6F6FE2'>(U) = Unban , (E) = Edit Ban</font> <font color='green'>(Total<HR><table border=1 rules=all frame=void cellspacing=0 cellpadding=3 </font>>"
	Banlistjob.cd = "/base"
	for (var/A in Banlistjob.dir)
		count++
		Banlistjob.cd = "/base/[A]"
...
/*/datum/admins/proc/permjobban(ckey, computerid, reason, bannedby, temp, minutes, rank)
	if(AddBanjob(ckey, computerid, reason, usr.ckey, 0, 0, job))
		...
		message_admins("<font color='#6F6FE2'>[usr.client.ckey] has banned from [job] [ckey].\nReason: [reason]\nThis is a permanent ban.</font>")
