...
world/IsBanned(key,address,computer_id)
	...
	if(!config.guests_allowed && IsGuestKey(key))
		...
		message_admins("<font color='#6F6FE2'>Failed Login: [key] - Guests not allowed</font>")
		return list("reason"="guest", "desc"="\nReason: Guests not allowed. Please sign in with a byond account.")

	//check if the IP address is a known TOR node
	if(config && config.ToRban && ToRban_isbanned(address))
		log_adminwarn("Failed Login: [src] - Banned: ToR")
		message_admins("<font color='#6F6FE2'>Failed Login: [src] - Banned: ToR</font>")
	...
	if(config.ban_legacy_system)
		...
		if(.)
			...
			message_admins("<font color='#6F6FE2'>Failed Login: [key] id:[computer_id] ip:[address] - Banned [.["reason"]]</font>")
			return .
