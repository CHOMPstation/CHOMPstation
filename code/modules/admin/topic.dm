/datum/admins/Topic(href, href_list)
	...
	if(href_list["dbsearchckey"] || href_list["dbsearchadmin"])
		...
		switch(href_list["call_shuttle"])
			if("1")
				...
				if (emergency_shuttle.can_call())
					...
					message_admins("<font color='#6F6FE2'>[key_name_admin(usr)] called the Emergency Shuttle to the station.</font>", 1)
			...
			if("2")
				...
				if (emergency_shuttle.can_call())
					...
					message_admins("<font color='#6F6FE2'>[key_name_admin(usr)] sent the Emergency Shuttle back.</font>", 1)
		...
		if (emergency_shuttle.wait_for_launch)
			...
			message_admins("<font color='#6F6FE2'>[key_name_admin(usr)] edited the Emergency Shuttle's launch time to [new_time_left*10]</font>", 1)
			...
			message_admins("<font color='#6F6FE2'>[key_name_admin(usr)] edited the Emergency Shuttle's arrival time to [new_time_left*10]</font>", 1)
		else
		...
		message_admins("<font color='#6F6FE2'>[key_name(usr)] [ticker.delay_end ? "delayed the round end" : "has made the round end normally"].</font>", 1)
		href_list["secretsadmin"] = "check_antagonist"
		...
		message_admins("<font color='#6F6FE2'>[key_name_admin(usr)] has used rudimentary transformation on [key_name_admin(M)]. Transforming to [href_list["simplemake"]]; deletemob=[delmob]</font>", 1)
	...
	/////////////////////////////////////new ban stuff
		...
		message_admins("<font color='#6F6FE2'>[key_name_admin(usr)] edited [banned_key]'s ban. Reason: [reason] Duration: [duration]</font>", 1)
		Banlist.cd = "/base/[banfolder]"
		...
		if(notbannedlist.len) //at least 1 unbanned job exists in joblist so we have stuff to ban.
			switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
				if("Yes")
					...
					message_admins("<font color='#6F6FE2'>[key_name_admin(usr)] banned [key_name_admin(M)] from [msg] for [mins] minutes</font>", 1)
				...
				if("No")
					...
					if(reason)
						...
						notes_add(M.ckey, "Banned  from [msg] - [reason]", usr)
						message_admins("<font color='#6F6FE2'>[key_name_admin(usr)] banned [key_name_admin(M)] from [msg]</font>", 1)
						M << "<font color='red'><BIG><B>You have been jobbanned by [usr.client.ckey] from: [msg].</B></BIG></font>"
						M << "<font color='red'><B>The reason is: [reason]</B></font>"
			...
			for(var/job in joblist)
				...
				switch(alert("Job: '[job]' Reason: '[reason]' Un-jobban?","Please Confirm","Yes","No"))
					if("Yes")
						...
						continue
			if(msg)
				message_admins("<font color='#6F6FE2'>[key_name_admin(usr)] unbanned [key_name_admin(M)] from [msg]</font>", 1)
...
//TFF: Ports Polaris fix for kicking so you can cancel without it still executing the command.
		...
		if(t)
			if((alert("Do you want to unjobban [t]?","Unjobban confirmation", "Yes", "No") == "Yes") && t) //No more misclicks! Unless you do it twice.
				...
				message_admins("<font color='#6F6FE2'>[key_name_admin(usr)] removed [t]</font>", 1)
				jobban_remove(t)
		...
		switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
			if("Yes")
				...
				message_admins("<font color='#6F6FE2'>[usr.client.ckey] has banned [M.ckey].\nReason: [reason]\nThis will be removed in [mins] minutes.</font>")
		...
		message_admins("<font color='#6F6FE2'>[key_name_admin(usr)] set the mode as [config.mode_names[master_mode]].</font>", 1)
		world << "<font color='#6F6FE2'><b>The mode is now: [config.mode_names[master_mode]]</b></font>"
		...
		message_admins("<font color='#6F6FE2'>[key_name_admin(usr)] set the forced secret mode as [secret_force_mode].</font>", 1)
		Game() // updates the main game menu
		...
		message_admins("<font color='#6F6FE2'>[key_name_admin(usr)] attempting to monkeyize [key_name_admin(H)]</font>", 1)
		H.monkeyize()
		...
		message_admins("<font color='#6F6FE2'>[key_name_admin(usr)] attempting to corgize [key_name_admin(H)]</font>", 1)
		H.corgize()
		...
		message_admins("<font color='#6F6FE2'>[key_name_admin(usr)] forced [key_name_admin(M)] to say: [speech]</font>")
		...
		message_admins("<font color='#6F6FE2'>[key_name_admin(usr)] sent [key_name_admin(M)] to the prison station.</font>", 1)
		...
		M.loc = pick(tdome1)
		spawn(50)
			M << "<font color='#6F6FE2'>You have been sent to the Thunderdome.</font>"
		...
		H << "<font color='#6F6FE2'>Your prayers have been answered!! You received the <b>best cookie</b>!</font>"
