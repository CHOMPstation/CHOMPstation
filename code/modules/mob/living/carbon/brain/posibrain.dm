...
/obj/item/device/mmi/digital/posibrain/attack_self(mob/user as mob)
	if(brainmob && !brainmob.key && searching == 0)
		...
		user << "<font color='#6F6FE2'>You carefully locate the manual activation switch and start the positronic brain's boot process.</font>"
...
/obj/item/device/mmi/digital/posibrain/proc/transfer_personality(var/mob/candidate)
	...
	for (var/mob/M in viewers(T))
		M.show_message("<font color='#6F6FE2'>The positronic brain beeps as it loads a personality.</font>")
...
/obj/item/device/mmi/digital/posibrain/proc/reset_search() //We give the players sixty seconds to decide, then reset the timer.
	...
	var/turf/T = get_turf_or_move(src.loc)
	for (var/mob/M in viewers(T))
		M.show_message("<font color='#6F6FE2'>The positronic brain buzzes and beeps, and the golden lights fade away. Perhaps you could try again?</font>")
