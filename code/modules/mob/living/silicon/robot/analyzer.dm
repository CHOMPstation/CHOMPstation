...
/obj/item/device/robotanalyzer/proc/do_scan(mob/living/M as mob, mob/living/user as mob)
	if((CLUMSY in user.mutations) && prob(50))
		...
		user.show_message(text("<font color='#6F6FE2'>Analyzing Results for The floor:\n\t Overall Status: Healthy</font>"), 1)
		user.show_message(text("<font color='#6F6FE2'>\t Damage Specifics: [0]-[0]-[0]-[0]</font>"), 1)
		user.show_message("<font color='#6F6FE2'>Key: Suffocation/Toxin/Burns/Brute</font>", 1)
		user.show_message("<font color='#6F6FE2'>Body Temperature: ???</font>", 1)
		return
	...
	switch(scan_type)
		if("robot")
			...
			user.show_message("<font color='#6F6FE2'>Analyzing Results for [M]:\n\t Overall Status: [M.stat > 1 ? "fully disabled" : "[M.health - M.halloss]% functional"]</font>")
			user.show_message("\t Key: <font color='#FFA500'>Electronics</font>/<font color='red'>Brute</font>", 1)
			user.show_message("\t Damage Specifics: <font color='#FFA500'>[BU]</font> - <font color='red'>[BR]</font>")
			if(M.tod && M.stat == DEAD)
				user.show_message("<font color='#6F6FE2'>Time of Disable: [M.tod]</font>")
			var/mob/living/silicon/robot/H = M
			var/list/damaged = H.get_damaged_components(1,1,1)
			user.show_message("<font color='#6F6FE2'>Localized Damage:</font>",1)
			if(length(damaged)>0)
				for(var/datum/robot_component/org in damaged)
					user.show_message(text("<font color='#6F6FE2'>\t []: [][] - [] - [] - []</font>",	\
					capitalize(org.name),					\
				...
				user.show_message("<font color='#6F6FE2'>\t Components are OK.</font>",1)
			if(H.emagged && prob(5))
				user.show_message("<font color='red'>\t ERROR: INTERNAL SYSTEMS COMPROMISED</font>",1)
			user.show_message("<font color='#6F6FE2'>Operating Temperature: [M.bodytemperature-T0C]&deg;C ([M.bodytemperature*1.8-459.67]&deg;F)</font>", 1)
