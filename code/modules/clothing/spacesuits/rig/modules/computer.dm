	...
	if(ai_mob)
		...
		if(ai_mob.key && ai_mob.client)
			...
			if(istype(ai, /obj/item/device/aicard))
				...
				ai.forceMove(src)
				ai_card = ai
				ai_mob << "<font color='#6F6FE2'>You have been transferred to \the [holder]'s [src].</font>"
				user << "<font color='#6F6FE2'>You load [ai_mob] into \the [holder]'s [src].</font>"
	...
	if(istype(input_device,/obj/item/weapon/disk/tech_disk))
		...
		if(disk.stored)
			if(load_data(disk.stored))
				user << "<font color='#6F6FE2'>Download successful; disk erased.</font>"
				disk.stored = null
			else
	...
	// I fucking hate R&D code. This typecheck spam would be totally unnecessary in a sane setup.
		...
		if(!incoming_files || !incoming_files.known_tech || !incoming_files.known_tech.len)
			...
			if(load_data(incoming_files.known_tech))
				user << "<font color='#6F6FE2'>Download successful; local and remote repositories synchronized.</font>"
			else
...
/obj/item/rig_module/power_sink/proc/drain_complete(var/mob/living/M)
	...
	if(!interfaced_with)
		if(M) M << "<font color='#6F6FE2'><b>Total power drained:</b> [round(total_power_drained*CELLRATE)] cell units.</font>"
	else
		if(M) M << "<font color='#6F6FE2'><b>Total power drained from [interfaced_with]:</b> [round(total_power_drained*CELLRATE)] cell units.</font>"
