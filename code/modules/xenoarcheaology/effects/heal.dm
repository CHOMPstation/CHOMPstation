...
/datum/artifact_effect/heal/DoEffectTouch(var/mob/toucher)
	...
	if(toucher && iscarbon(toucher))
		...
		if(prob(weakness * 100))
			...
			C << "<font color='#6F6FE2'>You feel a soothing energy invigorate you.</font>"
...
/datum/artifact_effect/heal/DoEffectAura()
	...
	if(holder)
		...
		for (var/mob/living/carbon/C in range(src.effectrange,T))
			...
			if(prob(weakness * 100))
				if(prob(10))
					C << "<font color='#6F6FE2'>You feel a soothing energy radiating from something nearby.</font>"
...
/datum/artifact_effect/heal/DoEffectPulse()
	...
	if(holder)
		...
		for (var/mob/living/carbon/C in range(src.effectrange,T))
			var/weakness = GetAnomalySusceptibility(C)
			if(prob(weakness * 100))
				C << "<font color='#6F6FE2'>A wave of energy invigorates you.</font>"
