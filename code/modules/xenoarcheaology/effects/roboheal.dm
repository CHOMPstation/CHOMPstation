...
/datum/artifact_effect/roboheal/DoEffectTouch(var/mob/user)
	if(user)
		if (istype(user, /mob/living/silicon/robot))
			...
			R << "<font color='#6F6FE2'>Your systems report damaged components mending by themselves!</font>"
	...
	if(holder)
		...
		for (var/mob/living/silicon/robot/M in range(src.effectrange,T))
			if(world.time - last_message > 200)
				M << "<font color='#6F6FE2'>SYSTEM ALERT: Beneficial energy field detected!</font>"
...
/datum/artifact_effect/roboheal/DoEffectPulse()
	if(holder)
		...
		for (var/mob/living/silicon/robot/M in range(src.effectrange,T))
			if(world.time - last_message > 200)
				M << "<font color='#6F6FE2'>SYSTEM ALERT: Structural damage has been repaired by energy pulse!</font>"
				last_message = world.time
