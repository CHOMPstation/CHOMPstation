...
/obj/item/weapon/anodevice/attackby(var/obj/I as obj, var/mob/user as mob)
	if(istype(I, /obj/item/weapon/anobattery))
		if(!inserted_battery)
			user << "<font color='#6F6FE2'>You insert the battery.</font>"
			user.drop_item()
			I.loc = src
			inserted_battery = I
			UpdateSprite()
	...
	if(activated)
		if(inserted_battery && inserted_battery.battery_effect && (inserted_battery.stored_charge > 0) )
			...
			if(inserted_battery.stored_charge <= 0)
				src.loc.visible_message("<font color='#6F6FE2'>\icon[src] [src] buzzes.</font>", "<font color='#6F6FE2'>\icon[src] You hear something buzz.</font>")
				shutdown_emission()
			else if(world.time > time_end)
				src.loc.visible_message("<font color='#6F6FE2'>\icon[src] [src] chimes.</font>", "<font color='#6F6FE2'>\icon[src] You hear something chime.</font>")
				shutdown_emission()
		else
			src.visible_message("<font color='#6F6FE2'>\icon[src] [src] buzzes.</font>", "<font color='#6F6FE2'>\icon[src] You hear something buzz.</font>")
			shutdown_emission()
...
/obj/item/weapon/anodevice/Topic(href, href_list)
	...
	if(href_list["startup"])
		if(inserted_battery && inserted_battery.battery_effect && (inserted_battery.stored_charge > 0) )
			activated = 1
			src.visible_message("<font color='#6F6FE2'>\icon[src] [src] whirrs.</font>", "\icon[src]<font color='#6F6FE2'>You hear something whirr.</font>")
			if(!inserted_battery.battery_effect.activated)
				inserted_battery.battery_effect.ToggleActivate(1)
...
/obj/item/weapon/anodevice/attack(mob/living/M as mob, mob/living/user as mob, def_zone)
	...
	if(activated && inserted_battery.battery_effect.effect == EFFECT_TOUCH && !isnull(inserted_battery))
		...
		inserted_battery.use_power(energy_consumed_on_touch)
		user.visible_message("<font color='#6F6FE2'>[user] taps [M] with [src], and it shudders on contact.</font>")
	else
		user.visible_message("<font color='#6F6FE2'>[user] taps [M] with [src], but nothing happens.</font>")
