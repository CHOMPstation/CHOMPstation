...
/obj/machinery/suspension_gen/interact(var/mob/user)
	...
	dat += "<hr>"
	dat += "<hr>"
	dat += "<font color='#6F6FE2'><b>Always wear safety gear and consult a field manual before operation.</b></font><br>"
	if(!locked)
...
/obj/machinery/suspension_gen/proc/activate()
	...
	for(var/mob/living/M in T)
		M.weakened += 5
		M.visible_message("<font color='#6F6FE2'>\icon[M] [M] begins to float in the air!</font>","You feel tingly and light, but it is difficult to move.")

	suspension_field = new(T)
	src.visible_message("<font color='#6F6FE2'>\icon[src] [src] activates with a low hum.</font>")
	...
	if(collected)
		suspension_field.icon_state = "energynet"
		suspension_field.overlays += "shield2"
		src.visible_message("<font color='#6F6FE2'>\icon[suspension_field] [suspension_field] gently absconds [collected > 1 ? "something" : "several things"].</font>")
	else
...
/obj/machinery/suspension_gen/proc/deactivate()
	...
	src.visible_message("<font color='#6F6FE2'>\icon[src] [src] deactivates with a gentle shudder.</font>")
