...
// Our analagous concept to piping_layer is connect_type
...
GLOBAL_LIST_INIT(atmos_pipe_recipes, list(
	...
	"Devices" = list(
		new /datum/pipe_info/pipe("Connector",			/obj/machinery/atmospherics/unary/portables_connector),
		...
		new /datum/pipe_info/pipe("Volume Pump",		/obj/machinery/atmospherics/binary/volume_pump),
		...
		new /datum/pipe_info/pipe("Gas Filter",			/obj/machinery/atmospherics/trinary/filter),
...
/obj/item/pipe_dispenser
	...
	matter = list(DEFAULT_WALL_MATERIAL = 40000, "glass" = 10000)
...
/obj/item/pipe_dispenser/New()
	...
	if(!first_atmos)
		first_atmos = global.atmos_pipe_recipes[global.atmos_pipe_recipes[1]][1]
	if(!first_disposal)
		first_disposal = global.disposal_pipe_recipes[global.disposal_pipe_recipes[1]][1]
	recipe = first_atmos
...
/obj/item/pipe_dispenser/ui_data(mob/user)
	...
	switch(screen)
		if(ATMOS_MODE)
			recipes = global.atmos_pipe_recipes
		if(DISPOSALS_MODE)
			recipes = global.disposal_pipe_recipes
	...
	for(var/c in global.pipe_colors)
		data["paint_colors"] += list(list("color_name" = c, "color_hex" = global.pipe_colors[c], "selected" = (c == paint_color)))
...
/obj/item/pipe_dispenser/ui_act(action, params)
	...
	switch(action)
		...
		if("pipe_type")
			...
			if(!recipes)
				recipes = global.disposal_pipe_recipes + global.atmos_pipe_recipes
...
/obj/item/pipe_dispenser/pre_attackby(atom/A, mob/user)
	...
	if(!make_pipe_whitelist)
		make_pipe_whitelist = typecacheof(list(/obj/structure/lattice, /obj/structure/girder, /obj/structure/catwalk, /obj/item/pipe))
	...
	switch(mode) //if we've gotten this var, the target is valid
		if(PAINT_MODE) //Paint pipes
			...
			P.change_color(global.pipe_colors[paint_color])
		...
		if(ATMOS_MODE) //Making pipes
			...
			if(do_after(user, 2, target = A))
				...
				P.add_atom_colour(global.pipe_colors[paint_color], FIXED_COLOUR_PRIORITY)
