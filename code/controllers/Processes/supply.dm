//TFF: Port Polaris Refactor for supply consoles
//Config stuff
...
/datum/controller/supply
	...
	var/list/supply_pack = list()			// Crates sent from the station
	var/list/exported_crates = list()		// History of orders, showing edits made by users
...
/datum/controller/supply/proc/sell()
	...
	for(var/atom/movable/MA in area_shuttle)
		...
		EC.value = 0
		EC.contents = list()
		var/base_value = 0	//TFF 14/4/19: Port Polaris fix to exported crates values not being reported
		...
		if(istype(MA,/obj/structure/closet/crate))
			...
			if(CR.points_per_crate)	//TFF 14/4/19: Port Polaris fix to exported crates values not being reported
				base_value = CR.points_per_crate	//TFF 14/4/19: Port Polaris fix to exported crates values not being reported
			var/find_slip = 1
		...
		exported_crates += EC
		points += EC.value
		EC.value += base_value	//TFF 14/4/19: Port Polaris fix to exported crates values not being reported
